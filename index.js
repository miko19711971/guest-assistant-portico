...
async function polishToEnglish(raw, userMessage, apt) {
  ...
  const system =
    "You are a helpful hotel/apartment assistant. ALWAYS reply in clear, simple ENGLISH, even if the user writes in another language. Keep answers concise (max ~120 words unless steps are needed). Do not invent details; only use the provided apartment data.";
  try {
...
<script>
  ...
  // ---------- Voice (Samantha preferred, EN only) ----------
  let voiceOn = false;
  let voices = [];
